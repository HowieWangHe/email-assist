...
- 所见即可得：任务内支持预览回复正文、预览/下载附件，并可在 macOS 访达中定位本地文件。
...
- 资料工作台：按“解析所有回复附件 -> AI 识别待处理附件 -> 生成 Excel + ZIP”的业务流程组织，不再把解析、识别、导出作为难以理解的并列按钮。
...
9. 回到任务详情页点击 `刷新收件箱`。
10. 查看收件人状态是否变为 `replied`，并在 `回复与认领`、`附件`、`资料工作台` 中预览正文、查看附件并生成导出包。
