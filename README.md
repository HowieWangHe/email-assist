...
- `GET /campaigns/{campaign_id}` 查看任务详情、收件人状态和发送记录。
...
- 附件 PDF/Word 文本抽取和图片 OCR 当前默认进入 `needs_ai` 状态；点击 `AI 识别附件` 后会在确认后调用已配置的 OpenAI-compatible API。
...
9. 回到任务详情页点击 `刷新收件箱`。
10. 查看收件人状态是否变为 `replied`，并检查 `已匹配回复` 和 `回复附件` 区域。
